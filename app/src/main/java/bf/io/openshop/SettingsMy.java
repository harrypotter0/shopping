...
public class SettingsMy {
    ...
    public static final String PREF_ACTUAL_LOCATION = "pref_actual_location";
    ...
    public static Location getActualLocation() {
        if (location != null) {
            ...
            SharedPreferences prefs = getSettings();
            String json = prefs.getString(PREF_ACTUAL_LOCATION, "");
            if (json.isEmpty() || "null".equals(json)) {
                Timber.e("%s - Returned null shop", TAG);
                return null;
            } else {
                location = Utils.getGsonParser().fromJson(json, Location.class);
                Timber.d("%s - Returned shop from memory: %s", TAG, location.toString());
                return location;
            }
        }
    }
     ...
     * Set actually selected shop.
     *
     * @param location selected shop or null for disable selection.
     */
    public static void setActualLocation(Location location) {
        if (actualShop != null)
            ...
            Timber.d("%s - Disable selected shop", TAG);
        SettingsMy.location = location;

        String json = Utils.getGsonParser().toJson(SettingsMy.location);
        SharedPreferences.Editor editor = getSettings().edit();
        editor.putString(PREF_ACTUAL_LOCATION, json);
        editor.apply();
    }
     ...
     */
    @NonNull
    public static Location getActualNonNullLocation(Activity activity) {
