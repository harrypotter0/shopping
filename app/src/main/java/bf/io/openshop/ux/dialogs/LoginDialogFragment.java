...
//            SettingsMy.setUserEmailHint(etRegistrationEmail.getText().toString());
    ...
    private void registerNewUser(EditText editTextEmail, EditText editTextPassword) {
        ...
        String url = String.format(EndPoints.USER_REGISTER, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/);
        progressDialog.show();
        ...
        GsonRequest<User> registerNewUser = new GsonRequest<>(Request.Method.POST, url, jo.toString(), User.class,
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Log.e("LoginDialog","onErrorResponse"+error);
    ...
    private void logInWithEmail(EditText editTextEmail, EditText editTextPassword) {
        SettingsMy.setUserEmailHint(editTextEmail.getText().toString());
        String url = String.format(EndPoints.USER_LOGIN_EMAIL, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/);
        progressDialog.show();
    ...
    private void resetPassword(EditText emailOfForgottenPassword) {
        String url = String.format(EndPoints.USER_RESET_PASSWORD, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/);
        progressDialog.show();
    ...
    private void verifyUserOnApi(JSONObject userProfileObject, AccessToken fbAccessToken) {
        String url = String.format(EndPoints.USER_LOGIN_FACEBOOK, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/);
        JSONObject jo = new JSONObject();
        try {
