        ...
        cartRecyclerAdapter = new CartRecyclerAdapter(getActivity(), new CartRecyclerInterface() {
            ...
            private void deleteItemFromCart(long id, boolean isDiscount) {
                ...
                if (user != null) {
                    String url;
                    if (isDiscount)
                        url = String.format(EndPoints.CART_DISCOUNTS_SINGLE, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/, id);
                    else
                        url = String.format(EndPoints.CART_ITEM, 21/*SettingsMy.getActualNonNullShop(getActivity()).getId()*/, id);
